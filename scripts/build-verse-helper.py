...
def main():
    ...
    def page(bands, n, subtitle, extra='', balanced=False, slim=False):
        head = (f'''<header class="slim">
      <div class="brand">UTKARSH <span>2026</span></div>
      <div class="title">Gita Shloka &middot; Judges' verse helper</div>
      <div class="sub">{subtitle}</div>
    </header>''' if slim else f'''<header>
      <div class="brand">UTKARSH <span>2026</span></div>
  ...
  <div class="page">
    ...
    <div class="cols{' balanced' if balanced else ''}{' tall' if slim else ''}">{''.join(band_html(b) for b in bands)}</div>{extra}
...
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  header.slim {{ padding-bottom: 1.4mm; margin-bottom: 2mm; border-bottom-width: 1.4pt;
                display: flex; justify-content: space-between; align-items: baseline; gap: 3mm; }}
  header.slim .brand {{ font-size: 9pt; letter-spacing: .12em; }}
  header.slim .title {{ font-size: 8.4pt; font-weight: 700; margin: 0; }}
  header.slim .sub {{ font-size: 7.4pt; margin: 0; white-space: nowrap; }}
  ...
  /* column-fill:auto needs a height to fill INTO — without one the columns keep
     ...
     ever added here, re-run and count them on the PDF before printing. The type
     sizes below are the comfortable ones; they only fit because sheet two runs a
     slim header, so shrink the type rather than the header if space runs short. */
  .cols {{ column-count: 2; column-gap: 7mm; column-fill: auto; height: 240mm; }}
  .cols.balanced {{ height: auto; column-fill: balance; }}
  .cols.tall {{ height: 253mm; }}
  ...
  .verse {{ break-inside: avoid; margin: 0 0 2.6mm; padding-left: 2mm; border-left: 1.6pt solid #E2C088; }}
  .ref {{ font-size: 8pt; font-weight: 700; color: #C1440E; letter-spacing: .04em; }}
  .sanskrit {{
    font-size: 8.6pt; line-height: 1.34; margin: .5mm 0 .7mm;
    font-style: italic; color: #14202b;
  }}
  .gist {{ font-size: 7.4pt; line-height: 1.3; color: #4b5563; }}
...
{page(page2, 2, 'Classes 5 to 10 &middot; groups B and C', slim=True)}
</body></html>"""
